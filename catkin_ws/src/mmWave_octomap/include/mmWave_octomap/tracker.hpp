...
GTRACK_gatingParams appGatingParamTable[2] = {
    ...
    {4.f, {2.f, 2.f, 0.f}}    /* PC: 2 gating volume, Limits are set to 2m in length, 2m in width, no limit in doppler */
};
GTRACK_stateParams appStateParamTable[2] = {
    {3U, 3U, 5U, 1000U, 5U},  /* TM: 3 det2act, 3 det2free, 5 act2free, 1000 stat2free, 5 exit2free*/
    {2U, 5U, 100U, 100U, 5U} /* PC: 3 det2act, 5 det2free, 100 act2free, 100 stat2free, 5 exit2free */
};
GTRACK_allocationParams appAllocationParamTable[2] = {
    ...
    {150.f, 250.f, 0.1f, 5U, 2.f, 2.f}  /* PC: SNRs 150 (250 obscured), 0.1 m/s minimal velocity, 5 points, with 1m in distance, 2m/c in velocity in separation */
};
...
ros::Publisher ego_velocity;
  ros::Publisher position_pub;
  void create_tracker();
  void publishMarker(uint32_t shape, std::string frame_id, uint32_t marker_id, uint32_t tid, float posX, float posY, float red, float blue, float green, float depth, float width);
